...
/* sched_ticks jest intem, wiec moze zadzaila z innym intem - sprawdz potem przemnozenie
dwoch intow przez siebie oraz dwoch definow */

PRIVATE int sched_ticks_table[3] = {a_time * SCHED_RATE, b_time * SCHED_RATE, c_time * SCHED_RATE};
/* a table that has quantums of time for each group of processes */
 ...
 */
  ...
  if (--sched_ticks_table[bill_ptr->p_group] == 0) {
	if (bill_ptr == prev_ptr) lock_sched();	/* process has run too long */
	sched_ticks_table[bill_ptr->p_group] = SCHED_RATE * sched_table[bill_ptr->p_group];		
/* reset quantum  #### based on variables inside sched table */
 ...
 */
  ...
  if (sched_ticks_table[bill_ptr->p_group] == 1) fd_timer();		/* floppy deselect */
#endif

  if (next_alarm <= now ||
      sched_ticks_table[bill_ptr->p_group] == 1 &&
      bill_ptr == prev_ptr &&
  ...
  if (--sched_ticks_table[bill_ptr->p_group] == 0) {
	...
	sched_ticks_table[bill_ptr->p_group] = SCHED_RATE * sched_table[bill_ptr->p_group];	/* reset quantum */
	prev_ptr = bill_ptr;		/* new previous process */
  }
