 ...
 */
  ...
  int next_group;	/* says which group is next in queue */
  ...
  next_group = ( rdy_head[USER_Q]->p_group + 1 ) % 3;
  ...
  p_next = NIL_PROC;
  while ( p_next == NIL_PROC )
  {
     tmp = rdy_head[USER_Q];
     p_prev = NIL_PROC;
     while ( tmp != NIL_PROC )
     {
	if ( tmp->p_group == next_group )
	{
	   p_next = tmp;
	   break;
	}
	...
	tmp = tmp->p_nextready;
     ...
     next_group = ( next_group + 1 ) % 3;
  }
  if ( p_next != rdy_head[USER_Q] )
  {
     if ( p_next == rdy_tail[USER_Q] )
	rdy_tail[USER_Q] = p_prev;
     p_prev->p_nextready = p_next->p_nextready;
     p_next->p_nextready = rdy_head[USER_Q];
     rdy_head[USER_Q] = p_next;
  }
